package com.example.imoocsdk.report;
...
import com.example.imoocsdk.okhttp.HttpConstant.Params;
...
public class ReportManager {
     ...
     * send the sueReoprt
     */
    public static void sueReport(ArrayList<Monitor> monitors,
                                 boolean isFull, long playTime) {
        if (monitors != null && monitors.size() > 0) {
            for (Monitor monitor : monitors) {
                RequestParams params = new RequestParams();
                if (Utils.containString(monitor.url, HttpConstant.ATM_PRE)) {
